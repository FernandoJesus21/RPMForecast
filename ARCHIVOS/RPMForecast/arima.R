...
# arima.R : Script que contiene la función para ejecutar modelos ARIMA
################################################################################

ejecutarARIMA <- function(ts_data, df_data, cv = F, steps){
  ...
  tryCatch({
    ...
    evaluate_arima <- function(train, h) {
      model <- auto.arima(train, max.p = 4, max.q = 4, lambda = NULL)
      # Realiza el forecast
    ...
    model_arima <- auto.arima(ts_data, max.p = 4, max.q = 4, lambda = NULL) #modelo propuesto
    ...
    data_arima$modelo <- "ARIMA"
    info(logger, paste0("ARIMA (", data_arima$serie[1], ") OK"))
    
    resultados <- list(data_arima, mae_per_step_arima, rmse_per_step_arima, model_arima)
    return(resultados)
  },
  error = function(e){
    error(logger, paste0("[arima.R] (", df_data$serie[1], ") ERROR: ", e))
    stop("Subproceso truncado. No se puede continuar.\n")
  },
  warning = function(e){
    warn(logger, paste0("[arima.R] (", df_data$serie[1], ") WARNING: ", e))
  })
